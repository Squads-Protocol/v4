...
describe("Versioned Multisig Permissions", () => {
    ...
    describe("Basic Permissions", () => {
        beforeEach(async () => {
            ...
            const initiateOnlyMember = helper.generateMembers(1, new Permissions({ initiate: true }), 0)[0];
            const voteOnlyMember = helper.generateMembers(1, new Permissions({ vote: true }), 0)[0];
            const executeOnlyMember = helper.generateMembers(1, new Permissions({ execute: true }), 0)[0];
        ...
        it("Enforces initiate permission for proposal creation", async () => {
            ...
            await helper.createVersionedProposal(multisigPda, initiateMember.keyPair, 0);
            ...
            try {
                ...
                await helper.createVersionedProposal(multisigPda, voteOnlyMember.keyPair, 1);
                assert.fail("Should not allow proposal creation without initiate permission");
            } catch (error: any) {
                assert(error.toString().includes("InvalidPermissions"));
            }
        });
        ...
        it("Enforces execute permission for execution", async () => {
            ...
            await helper.executeVaultTransaction(multisigPda, 0, executeMember.keyPair);
            ...
            try {
                await helper.executeVaultTransaction(multisigPda, 1, initiateOnlyMember.keyPair);
        ...
        it("Handles permission combinations correctly", async () => {
            ...
            const initiateAndVoteMember = helper.generateMembers(1, new Permissions({ initiate: true, vote: true }), 0)[0];
            ...
            await helper.createVersionedVaultTransaction(multisigPda, initiateAndVoteMember.keyPair);
            const { proposalPda } = await helper.createVersionedProposal(multisigPda, initiateAndVoteMember.keyPair, 0);
            await helper.vote(multisigPda, proposalPda, initiateAndVoteMember.keyPair, true);

            // But should not be able to execute
            try {
                await helper.executeVaultTransaction(multisigPda, 0, initiateAndVoteMember.keyPair);
                assert.fail("Should not allow execution without execute permission");
            } catch (error: any) {
                assert(error.toString().includes("InvalidPermissions"));
            }
